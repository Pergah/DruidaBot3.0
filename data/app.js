...
// Estado ON/OFF seteado manualmente — protege el visual de ser sobreescrito por el servidor
// durante 5 segundos (tiempo suficiente para que el firmware confirme el estado real)
const pendingRelayOn = {}; // { relayIdx: { on: bool, until: ms } }
function setPendingOn(i, on) {
  pendingRelayOn[i] = { on: !!on, until: Date.now() + 5000 };
}
// Devuelve el ON/OFF a mostrar: prioriza el estado pendiente si aún no expiró
function resolveOn(i, serverOn) {
  const p = pendingRelayOn[i];
  if (p && Date.now() < p.until) return p.on;
  delete pendingRelayOn[i];
  return serverOn;
}
...
const MODE_TEXT = {
  ...
  'auto': 'Auto', 'manual': 'Manual', 'timer': 'Timer', 'ir': 'IR A/C'
};
...
function reasonForRelay(r, params) {
  ...
  if (kind === 'ir') {
    ...
    return `IR A/C · enciende frío si T > ${p.tempAcOn}° · apaga si T < ${p.tempAcOff}°`;
  }
  if (kind === 'timer')  {
...
function renderDashRelays(state) {
  ...
  for (let i = 1; i <= 8; i++) {
    const r   = relays['r' + i] || {};
    const on  = resolveOn(i, r.on);   // respeta estado pendiente (manual click)
    const name = r.name || RELAY_DEFAULT_NAMES[i] || ('R' + i);
    const icon = RELAY_ICONS[i] || '⚡';
    const cfg  = currentRelayCfg['r' + i] || {};
    const kind = cfg.mode || numericModeToKind(r.mode) || 'auto';
    const modeLabel = kind === 'auto' ? 'auto' : kind === 'timer' ? 'timer' : kind === 'ir' ? 'IR A/C' : 'manual';
    html += `
...
function renderDevices() {
  ...
  for (let i = 1; i <= 8; i++) {
    ...
    const baseModes = autoManualOnly ? ['auto','manual'] : ['auto','manual','timer'];
    ...
    const modeBtns = modes.map(k => `
      <button type="button" class="mode-btn ${activeMode === k ? 'sel' : ''}"
        ...
        ${k === 'auto' ? (useTimerLabel ? 'Timer' : 'Auto') : k === 'manual' ? 'Manual' : k === 'ir' ? '❄️ IR A/C' : 'Timer'}
      </button>
    `).join('');
      ...
      extra = `
        <div class="mode-extra">
          <label class="irrig-param-row" style="cursor:pointer;gap:10px;align-items:center">
            <span class="timer-label">Todo el día</span>
      ...
      extra = `
        <div class="mode-extra">
          <div class="mode-hint ir">
            ❄️ IR A/C: enciende frío si T &gt; <b>${p.tempAcOn}°C</b> · apaga si T &lt; <b>${p.tempAcOff}°C</b><br>
            <span style="opacity:.7">Configurá los umbrales en la pestaña <b>Parámetros</b></span>
          </div>
        </div>`;
    ...
    cards.push(`
      <div class="relay-card ${on ? 'is-on' : ''}">
        ...
        ${extra}
        ${dirtyRelayIds.has(i) ? `<button type="button" class="save-appear" onclick="saveRelayToFirmware(${i},'Guardado')">Guardar</button>` : ''}
      </div>
    `);
  }
...
function setRelayMode(i, kind) {
  ...
  // Actualización optimista del modeKind en lastState para que el badge del dashboard también se actualice
  if (lastState?.relays?.['r' + i]) {
    lastState.relays['r' + i].modeKind = kind;
  }
  renderDevices();
  if (lastState) renderDashRelays(lastState);
  saveRelayToFirmware(i, 'Modo guardado');
}
...
function irrigValidation(cfg) {
  ...
  if (cfg.todoElDia) {
    const pausaSec = (cfg.pausaVal ?? 10) * (cfg.pausaUnit ?? 60);
    if (pausaSec <= 0) return 'La pausa debe ser mayor a 0 para "todo el día"';
  }
  return null;
}
...
function setManualState(i, on) {
  ...
  // Protege el visual 5 s contra sobreescritura del servidor
  setPendingOn(i, on);

  // Parchea lastState si existe (evita que el re-render post-loadState muestre valor viejo)
  if (lastState) {
  ...
  // Actualiza visual SIEMPRE (pendingRelayOn garantiza el valor correcto
  // incluso si lastState es null o el firmware aún devuelve on:false)
  renderDashRelays(lastState);
  renderDevices();

  // Enviar al firmware y refrescar estado real
...
async function saveAllRelays() {
  ...
  try {
    const ids = dirtyRelayIds.size ? Array.from(dirtyRelayIds) : [1,2,3,4,5,6,7,8];
    for (const i of ids) await postRelay(i);
...
async function postRelay(i) {
  ...
  } else if (ILLUMINATION_RELAYS.has(i) && modeToSend === 'auto') {
    payloadOnTime = String(cfg.horaOn ?? 0).padStart(2, '0') + ':' + String(cfg.minOn ?? 0).padStart(2, '0');
    payloadOffTime = String(cfg.horaOff ?? 0).padStart(2, '0') + ':' + String(cfg.minOff ?? 0).padStart(2, '0');
  }
...
async function connectWifi() {
  ...
  if (!ssid) { showToast('Ingresá el nombre de la red', true); return; }
  const ok = confirm(
    `¿Conectar el Druida a "${ssid}"?\n\n` +
    'Se guardarán las credenciales y el dispositivo intentará conectarse.\n' +
    'Si falla, volverá al modo AP (192.168.4.1).'
  );
  if (!ok) return;
  if (msgEl) { msgEl.textContent = 'Guardando credenciales…'; msgEl.classList.remove('hidden'); }
  try {
    const body = new URLSearchParams({ ssid, pass });
    ...
    if (msgEl) msgEl.textContent = 'Conectando…';
