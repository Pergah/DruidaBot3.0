// =========================
// Druida BOT â€” frontend
// =========================
function byId(id) { return document.getElementById(id); }
window.druidaToast = showToast;

// =========================
// Toast
// =========================
function showToast(msg, isErr = false) {
  const t = byId('actionToast');
  if (!t) return;
  t.textContent = msg || '';
  t.classList.remove('err');
  if (isErr) t.classList.add('err');
  t.classList.add('show');
  clearTimeout(window.__toastTimer);
  window.__toastTimer = setTimeout(() => t.classList.remove('show'), 1800);
}

// =========================
// NavegaciÃ³n SPA + tabs
// =========================
function setView(mode) {
  const views = ['homeView','devicesView','paramsView'];
  views.forEach(id => {
    const el = byId(id);
    if (el) el.classList.add('preloaded-hidden');
  });
  const target = byId(mode);
  if (target) target.classList.remove('preloaded-hidden');

  document.querySelectorAll('.tab').forEach(t => {
    t.classList.toggle('active', t.dataset.tab === mode);
  });

  window.scrollTo({ top: 0, behavior: 'instant' in window ? 'instant' : 'auto' });
}

function showPreloadedHome()    { setView('homeView'); }
function showPreloadedDevices() {
  setView('devicesView');
  renderDevices();
  loadState();
}
function showPreloadedParams()  { setView('paramsView');  renderParams(); }

// alias legacy (por si el firmware aÃºn los referencia)
function showPreloadedControl() { showPreloadedDevices(); }
function showPreloadedConfig()  { showPreloadedParams(); }
function showPreloadedControlR1(){ showPreloadedDevices(); }
function showPreloadedControlR2(){ showPreloadedDevices(); }
function showPreloadedControlR3(){ showPreloadedDevices(); }
function showPreloadedControlR4(){ showPreloadedDevices(); }
function showPreloadedControlR5(){ showPreloadedDevices(); }
function showPreloadedControlR6(){ showPreloadedDevices(); }
function showPreloadedConfigR1(){ showPreloadedParams(); }
function showPreloadedConfigR2(){ showPreloadedParams(); }
function showPreloadedConfigR3(){ showPreloadedParams(); }
function showPreloadedConfigR4(){ showPreloadedParams(); }
function showPreloadedConfigR5(){ showPreloadedParams(); }
function showPreloadedConfigR6(){ showPreloadedParams(); }
function showPreloadedConfigWiFi(){ showPreloadedParams(); }

// =========================
// Quick action / submit
// =========================
async function quickAction(url, label) {
  showToast(label || 'Procesando...');
  try {
    if (!url.includes('ajax=1')) url += (url.includes('?') ? '&' : '?') + 'ajax=1';
    const res = await fetch(url, { method: 'POST', headers: { 'X-Requested-With': 'XMLHttpRequest' } });
    if (!res.ok) throw new Error();
    const txt = await res.text();
    showToast(txt || (label || '') + ' OK');
    loadState();
  } catch {
    showToast('Error en acciÃ³n', true);
  }
}

async function submitFormConfig(formId, msg) {
  const f = byId(formId);
  if (!f) return false;
  showToast(msg || 'Guardando...');
  try {
    let url = f.action || window.location.pathname;
    if (!url.includes('ajax=1')) url += (url.includes('?') ? '&' : '?') + 'ajax=1';
    const res = await fetch(url, {
      method: 'POST',
      body: new FormData(f),
      headers: { 'X-Requested-With': 'XMLHttpRequest' }
    });
    const txt = (await res.text()).trim();
    showToast(txt || 'ConfiguraciÃ³n guardada');
  } catch {
    showToast('Error al guardar', true);
  }
  return false;
}

// =========================
// State client-side (cache + fallback)
// =========================
const PARAMS_KEY  = 'druida_params_v1';
const RELAYCFG_KEY = 'druida_relaycfg_v1';

const DEFAULT_PARAMS = {
  tempHeaterOn: 18, tempHeaterOff: 22, tempAcOff: 25, tempAcOn: 28,
  humHumidOn: 40,  humHumidOff: 55,  humDehumOff: 65, humDehumOn: 75,
  acRelay: 5, heaterRelay: 6, humidRelay: 1, dehumidRelay: 2
};

const QUAD_CFG = {
  temp: { min: 15, max: 35, step: 0.5, gapOuter: 3,  gapInner: 1,  fmt: v => v.toFixed(1) + ' Â°C' },
  hum:  { min: 30, max: 90, step: 1,   gapOuter: 10, gapInner: 5,  fmt: v => Math.round(v) + ' %'  }
};
const QUAD_KEYS = {
  temp: ['tempHeaterOn', 'tempHeaterOff', 'tempAcOff', 'tempAcOn'],
  hum:  ['humHumidOn',  'humHumidOff',  'humDehumOff', 'humDehumOn']
};

const RELAY_DEFAULT_NAMES = {
  1: 'HumidificaciÃ³n',
  2: 'ExtracciÃ³n',
  3: 'IrrigaciÃ³n 1',
  4: 'IluminaciÃ³n 1',
  5: 'Aire Acondicionado',
  6: 'CalefacciÃ³n',
  7: 'IluminaciÃ³n 2',
  8: 'IrrigaciÃ³n 2'
};

const DEFAULT_RELAY_CFG = () => {
  const out = {};
  for (let i = 1; i <= 8; i++) {
    out['r' + i] = {
      mode: 'auto', on: false, onTime: '06:00', offTime: '22:00',
      horaOn: 8, minOn: 0, horaOff: 22, minOff: 0,
      pulsoVal: 30, pulsoUnit: 1, pausaVal: 10, pausaUnit: 60, ciclos: 4,
      todoElDia: false,
      diasRiego: [false, true, true, true, true, true, false]
    };
  }
  return out;
};

function loadParams() {
  try {
    const raw = localStorage.getItem(PARAMS_KEY);
    if (raw) return Object.assign({}, DEFAULT_PARAMS, JSON.parse(raw));
  } catch {}
  return Object.assign({}, DEFAULT_PARAMS);
}
function saveParamsLocal(p) {
  try { localStorage.setItem(PARAMS_KEY, JSON.stringify(p)); } catch {}
}
function loadRelayCfg() {
  return {};
}
function saveRelayCfgLocal(c) {
  // La fuente persistente de relays es el ESP32. localStorage no debe sobrevivir a F5
  // con valores viejos que pisen lo que informa /api/state.
}

let lastState = null;
let currentParams = loadParams();
let currentRelayCfg = loadRelayCfg();
let dirtyParams = false;
const dirtyRelayIds = new Set();
const pendingRelayUi = {};
let devicesUiHoldUntil = 0;

function parseHHMM(v, fh = 8, fm = 0) {
  const m = String(v || '').match(/^(\d{1,2}):(\d{2})$/);
  if (!m) return { h: fh, m: fm };
  return {
    h: Math.max(0, Math.min(23, parseInt(m[1], 10))),
    m: Math.max(0, Math.min(59, parseInt(m[2], 10)))
  };
}

function relayCfgFromServer(i) {
  const r = (lastState && lastState.relays && lastState.relays['r' + i]) || {};
  const onParts = parseHHMM(r.onTime, 8, 0);
  const offParts = parseHHMM(r.offTime, 22, 0);
  return {
    mode: r.modeKind || numericModeToKind(r.mode) || 'auto',
    on: !!r.on,
    name: r.name || '',
    onTime: r.onTime || '06:00',
    offTime: r.offTime || '22:00',
    horaOn: r.horaOn ?? onParts.h,
    minOn: r.minOn ?? onParts.m,
    horaOff: r.horaOff ?? offParts.h,
    minOff: r.minOff ?? offParts.m,
    pulsoVal: r.pulsoVal ?? 30,
    pulsoUnit: r.pulsoUnit ?? 1,
    pausaVal: r.pausaVal ?? 10,
    pausaUnit: r.pausaUnit ?? 60,
    ciclos: r.ciclos ?? 4,
    todoElDia: !!r.todoElDia,
    diasRiego: Array.isArray(r.diasRiego) ? r.diasRiego.map(Boolean) : [false,true,true,true,true,true,false]
  };
}

function getRelayCfg(i) {
  return Object.assign({}, relayCfgFromServer(i), currentRelayCfg['r' + i] || {});
}

function holdDevicesUi(ms = 2500) {
  devicesUiHoldUntil = Date.now() + ms;
}

function devicesViewIsOpen() {
  const el = byId('devicesView');
  return !!el && !el.classList.contains('preloaded-hidden');
}

function devicesUiIsBusy() {
  if (!devicesViewIsOpen()) return false;
  const active = document.activeElement;
  const editing = active && byId('devicesList') && byId('devicesList').contains(active)
    && /^(INPUT|SELECT|TEXTAREA|BUTTON)$/.test(active.tagName);
  return !!editing || dirtyRelayIds.size > 0 || Date.now() < devicesUiHoldUntil;
}

function refreshDeviceRelayVisuals(state) {
  const relays = (state && state.relays) || {};
  for (let i = 1; i <= 8; i++) {
    const card = document.querySelector(`.relay-card[data-relay-idx="${i}"]`);
    if (!card) continue;
    const r = relays['r' + i] || {};
    const on = !!r.on;
    card.classList.toggle('is-on', on);
    const dot = card.querySelector('.relay-state-dot');
    if (dot) dot.classList.toggle('on', on);
    const status = card.querySelector('.relay-status');
    if (status) status.textContent = reasonForRelay({ ...r, idx: i, modeKind: getRelayCfg(i).mode, on }, currentParams);
  }
}

function normalizeModeKind(mode) {
  if (mode == null) return null;
  const s = String(mode).trim().toLowerCase();
  if (s === 'manual' || s === '1') return 'manual';
  if (s === 'auto' || s === 'automatico' || s === '2') return 'auto';
  if (s === 'timer' || s === 'por horario' || s === '6') return 'timer';
  if (s === 'ir' || s === 'ir a/c' || s === '10') return 'ir';
  if (s === 'superciclo1313' || s === '13/13' || s === '13') return 'superciclo1313';
  return null;
}

function modeFromRelayState(r) {
  return normalizeModeKind(r && (r.modeKind ?? r.mode)) || numericModeToKind(r && r.mode) || 'auto';
}

function setRelayVisualPending(i, on) {
  pendingRelayUi[i] = { on: !!on, until: Date.now() + 3000 };
}

function relayOnForDisplay(i, relay) {
  const serverOn = relay && relay.on;
  const pending = pendingRelayUi[i];
  if (!pending) return !!serverOn;
  if (serverOn !== undefined && !!serverOn === pending.on) {
    delete pendingRelayUi[i];
    return !!serverOn;
  }
  if (Date.now() < pending.until) return pending.on;
  delete pendingRelayUi[i];
  return !!serverOn;
}

function mergeIncomingState(s) {
  const previous = lastState || {};
  const merged = Object.assign({}, previous, s || {});
  const incomingRelays = (s && s.relays && typeof s.relays === 'object') ? s.relays : {};
  const hasIncomingRelays = Object.keys(incomingRelays).length > 0;
  const prevRelays = hasIncomingRelays ? {} : (previous.relays || {});
  const relays = hasIncomingRelays ? {} : Object.assign({}, prevRelays);

  for (let i = 1; i <= 8; i++) {
    const key = 'r' + i;
    const prevRelay = prevRelays[key] || {};
    const incomingRelay = incomingRelays[key] || {};
    const nextRelay = Object.assign({}, prevRelay, incomingRelay);
    nextRelay.on = relayOnForDisplay(i, nextRelay);
    relays[key] = nextRelay;
  }

  merged.relays = relays;
  return merged;
}

// Si el firmware envÃ­a params/configs en /api/state, los preferimos.
function ingestServerState(s) {
  // Preservar irConfigured en lastState para que renderDevices lo vea
  if (s && s.irConfigured !== undefined && lastState) {
    lastState.irConfigured = s.irConfigured;
  }
  if (s && s.params && typeof s.params === 'object') {
    if (!dirtyParams) {
      currentParams = Object.assign({}, DEFAULT_PARAMS, s.params);
      saveParamsLocal(currentParams);
    }
  }
  if (false && s && s.relays && typeof s.relays === 'object') {
    let touched = false;
    for (let i = 1; i <= 8; i++) {
      const r = s.relays['r' + i];
      if (!r) continue;
      if (dirtyRelayIds.has(i)) {
        continue;
      }
      const cfg = {};
      // El firmware puede enviar modeKind ("auto"/"manual"/"timer") o mode numÃ©rico.
      if (r.modeKind) { cfg.mode = r.modeKind; touched = true; }
      else if (r.mode !== undefined) {
        const k = numericModeToKind(r.mode);
        if (k) { cfg.mode = k; touched = true; }
      }
      if (r.onTime)  { cfg.onTime  = r.onTime;  touched = true; }
      if (r.offTime) { cfg.offTime = r.offTime; touched = true; }
      if (r.horaOn  !== undefined) { cfg.horaOn  = parseInt(r.horaOn,  10); touched = true; }
      if (r.minOn   !== undefined) { cfg.minOn   = parseInt(r.minOn,   10); touched = true; }
      if (r.horaOff !== undefined) { cfg.horaOff = parseInt(r.horaOff, 10); touched = true; }
      if (r.minOff  !== undefined) { cfg.minOff  = parseInt(r.minOff,  10); touched = true; }
      if (IRRIGATION_RELAYS.has(i)) {
        if (r.pulsoVal  !== undefined) { cfg.pulsoVal  = +r.pulsoVal;  touched = true; }
        if (r.pulsoUnit !== undefined) { cfg.pulsoUnit = +r.pulsoUnit; touched = true; }
        if (r.pausaVal  !== undefined) { cfg.pausaVal  = +r.pausaVal;  touched = true; }
        if (r.pausaUnit !== undefined) { cfg.pausaUnit = +r.pausaUnit; touched = true; }
        if (r.ciclos     !== undefined) { cfg.ciclos    = +r.ciclos;    touched = true; }
        if (r.todoElDia  !== undefined) { cfg.todoElDia = !!r.todoElDia; touched = true; }
        if (Array.isArray(r.diasRiego)) { cfg.diasRiego = r.diasRiego.map(Boolean); touched = true; }
      }
      if (r.on !== undefined) cfg.on = !!r.on;
    }
    if (touched) saveRelayCfgLocal(currentRelayCfg);
  }
}

function numericModeToKind(m) {
  const s = String(m);
  if (s === '1')  return 'manual';
  if (s === '2')  return 'auto';
  if (s === '6')  return 'timer';
  if (s === '10') return 'ir';
  if (s === '13') return 'superciclo1313';
  return null;
}
function kindToNumericMode(k) {
  if (k === 'manual') return 1;
  if (k === 'auto')   return 2;
  if (k === 'timer')  return 6;
  if (k === 'ir')     return 10;
  if (k === 'superciclo1313') return 13;
  return 0;
}

// =========================
// Iconos / etiquetas
// =========================
const RELAY_ICONS = {
  1: 'ðŸ’¦', // HumidificaciÃ³n
  2: 'ðŸ’¨', // ExtracciÃ³n
  3: 'ðŸš¿', // IrrigaciÃ³n 1
  4: 'â˜€ï¸', // IluminaciÃ³n 1
  5: 'â„ï¸', // A/C
  6: 'ðŸ”¥', // CalefacciÃ³n
  7: 'ðŸ’¡', // IluminaciÃ³n 2
  8: 'ðŸš¿'  // IrrigaciÃ³n 2
};
const AUTO_MANUAL_ONLY_RELAYS = new Set([2, 3, 4, 7, 8]);
const TIMER_LABEL_RELAYS = new Set([4, 7]);
const ILLUMINATION_RELAYS = new Set([4, 7]);
const IRRIGATION_RELAYS   = new Set([3, 8]);

const MODE_TEXT = {
  '0': 'Apagado', '1': 'Manual', '2': 'Auto',
  '4': 'Ciclo',   '6': 'Timer',  '9': 'Auto VPD', '10': 'IR A/C', '13': 'Superciclo',
  'auto': 'Auto', 'manual': 'Manual', 'timer': 'Timer', 'ir': 'IR A/C', 'superciclo1313': '13/13'
};

function modeToText(mode) {
  if (mode == null || mode === '' || mode === '--') return '--';
  const s = String(mode).trim();
  return MODE_TEXT[s] || s;
}

function reasonForRelay(r, params) {
  if (r && r.reason) return String(r.reason);
  const kind = modeFromRelayState(r);
  if (kind === 'manual') return r.on ? 'Manual Â· encendido' : 'Manual Â· apagado';
  if (kind === 'ir') {
    const p = params || currentParams;
    return `IR A/C Â· enciende frÃ­o si T > ${p.tempAcOn}Â° Â· apaga si T < ${p.tempAcOff}Â°`;
  }
  if (kind === 'timer')  {
    const cfg = getRelayCfg(r.idx);
    return `Timer Â· ${cfg.onTime || '--:--'} â†’ ${cfg.offTime || '--:--'}`;
  }
  if (kind === 'superciclo1313') {
    const cfg = getRelayCfg(r.idx);
    return `13/13 Â· ${cfg.onTime || '--:--'} -> ${cfg.offTime || '--:--'}`;
  }
  if (kind === 'auto') {
    if (!params) return r.on ? 'Auto Â· activo' : 'Auto Â· en espera';
    const i = r.idx;
    if (params.acRelay      == i) return r.on ? `Auto Â· T > ${params.tempAcOn}Â° â†’ OFF a ${params.tempAcOff}Â°` : `Auto Â· enciende si T > ${params.tempAcOn}Â°`;
    if (params.heaterRelay  == i) return r.on ? `Auto Â· T < ${params.tempHeaterOn}Â° â†’ OFF a ${params.tempHeaterOff}Â°` : `Auto Â· enciende si T < ${params.tempHeaterOn}Â°`;
    if (params.humidRelay   == i) return r.on ? `Auto Â· HR < ${params.humHumidOn}% â†’ OFF a ${params.humHumidOff}%` : `Auto Â· enciende si HR < ${params.humHumidOn}%`;
    if (params.dehumidRelay == i) return r.on ? `Auto Â· HR > ${params.humDehumOn}% â†’ OFF a ${params.humDehumOff}%` : `Auto Â· enciende si HR > ${params.humDehumOn}%`;
    return r.on ? 'Auto Â· activo' : 'Auto Â· sin asignaciÃ³n';
  }
  return modeToText(r && r.mode);
}

function relayPills(r) {
  const out = [];
  const kind = modeFromRelayState(r);
  if (kind === 'auto')   out.push({ text: 'AutomÃ¡tico', kind: 'blue' });
  if (kind === 'timer')  out.push({ text: 'Por horario', kind: 'purple' });
  if (kind === 'superciclo1313') out.push({ text: '13/13', kind: 'purple' });
  if (kind === 'manual') out.push({ text: 'Manual',     kind: 'amber' });
  if (kind === 'ir')     out.push({ text: 'IR A/C',     kind: 'teal' });
  if (r && r.on === true)  out.push({ text: 'Encendido', kind: 'green' });
  if (r && r.on === false) out.push({ text: 'Apagado',   kind: '' });
  return out;
}

function relayModeButtonText(k, useTimerLabel) {
  if (k === 'auto') return useTimerLabel ? 'Timer' : 'Auto';
  if (k === 'manual') return 'Manual';
  if (k === 'ir') return 'IR A/C';
  if (k === 'superciclo1313') return '13/13';
  return 'Timer';
}

function escapeHtml(s) {
  return String(s == null ? '' : s)
    .replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;')
    .replace(/"/g, '&quot;').replace(/'/g, '&#39;');
}

// =========================
// Render dashboard de relÃ©s (grid 3Ã—3)
// =========================
function renderDashRelays(state) {
  const host = byId('dashRelays');
  if (!host) return;
  const relays = (state && state.relays) || {};

  let html = '';
  for (let i = 1; i <= 8; i++) {
    const r   = relays['r' + i] || {};
    const on  = !!r.on;
    const name = r.name || RELAY_DEFAULT_NAMES[i] || ('R' + i);
    const icon = RELAY_ICONS[i] || 'âš¡';
    const cfg  = getRelayCfg(i);
    const kind = cfg.mode || modeFromRelayState(r);
    const modeLabel = kind === 'auto' ? 'auto' : kind === 'timer' ? 'timer' : kind === 'ir' ? 'IR A/C' : kind === 'superciclo1313' ? '13/13' : 'manual';
    html += `
      <div class="relay-btn ${on ? 'is-on' : ''}">
        <span class="relay-btn-dot ${on ? 'on' : ''}"></span>
        <span class="relay-btn-icon">${icon}</span>
        <span class="relay-btn-name">${escapeHtml(name)}</span>
        <span class="relay-btn-mode">${modeLabel}</span>
      </div>`;
  }
  // 9Âª celda vacÃ­a (para completar la grilla 3Ã—3)
  html += '<div class="relay-btn" style="opacity:0;pointer-events:none"></div>';
  host.innerHTML = html;

  // mantener legacy ids
  for (let i = 1; i <= 8; i++) {
    const r  = relays['r' + i] || {};
    const on = !!r.on;
    setText(`nameR${i}`, r.name || RELAY_DEFAULT_NAMES[i] || `R${i}`);
    setText(`modeR${i}Badge`, modeToText(r.mode));
    const dot = byId(`dotR${i}`);
    if (dot) { dot.classList.remove('on', 'off'); dot.classList.add(on ? 'on' : 'off'); }
  }
}

// =========================
// Render DISPOSITIVOS (modo por relÃ©)
// =========================
function renderDevices() {
  const host = byId('devicesList');
  if (!host) return;
  const relays = (lastState && lastState.relays) || {};

  const cards = [];
  for (let i = 1; i <= 8; i++) {
    const r = relays['r' + i] || {};
    const cfg = getRelayCfg(i);
    const name = r.name || RELAY_DEFAULT_NAMES[i] || ('R' + i);
    const icon = RELAY_ICONS[i] || 'âš¡';

    const autoManualOnly = AUTO_MANUAL_ONLY_RELAYS.has(i);
    const useTimerLabel = TIMER_LABEL_RELAYS.has(i);
    const irConfigured = !!(lastState && lastState.irConfigured);
    const supportsIR = (i === 2) && irConfigured;
    const baseModes = ILLUMINATION_RELAYS.has(i) ? ['auto','manual','superciclo1313']
                    : autoManualOnly ? ['auto','manual'] : ['auto','manual','timer'];
    const modes = supportsIR ? ['auto','manual','ir'] : baseModes;
    const activeMode = (autoManualOnly && !supportsIR && cfg.mode === 'timer') ? 'auto'
                     : (i === 2 && cfg.mode === 'ir' && !irConfigured) ? 'auto'
                     : cfg.mode;
    const modeBtns = modes.map(k => `
      <button type="button" class="mode-btn ${activeMode === k ? 'sel' : ''}"
              onclick="setRelayMode(${i}, '${k}')">
        ${relayModeButtonText(k, useTimerLabel)}
      </button>
    `).join('');

    let extra = '';
    if (activeMode === 'auto' && IRRIGATION_RELAYS.has(i)) {
      const pad2 = n => String(n == null ? 0 : n).padStart(2, '0');
      const onClock = `${pad2(cfg.horaOn ?? 8)}:${pad2(cfg.minOn ?? 0)}`;
      const pVal   = cfg.pulsoVal  ?? 30;
      const pUnit  = cfg.pulsoUnit ?? 1;
      const paVal  = cfg.pausaVal  ?? 10;
      const paUnit = cfg.pausaUnit ?? 60;
      const cics   = cfg.ciclos    ?? 4;
      const tdEl   = !!cfg.todoElDia;
      const dias   = Array.isArray(cfg.diasRiego) ? cfg.diasRiego : [false,true,true,true,true,true,false];
      const { h: offH, m: offM } = calcIrrigOff(cfg);
      const pulsoSec = pVal * pUnit;
      const durTxt = formatDuration(pulsoSec);
      const validErr = irrigValidation(cfg);
      const efectCiclos = tdEl ? calcCiclosTodoElDia(cfg) : cics;
      extra = `
        <div class="mode-extra">
          <label class="irrig-param-row" style="cursor:pointer;gap:10px;align-items:center">
            <span class="timer-label">Todo el dÃ­a</span>
            <input type="checkbox" ${tdEl ? 'checked' : ''} onchange="setIrrigTodoElDia(${i},this.checked)"
                   style="width:16px;height:16px;cursor:pointer">
          </label>
          ${!tdEl ? `
          <div class="timer-grid">
            <div class="timer-box">
              <div class="timer-label">Hora inicio</div>
              <input type="time" value="${escapeHtml(onClock)}"
                     onchange="setIrrigTime(${i}, this.value)">
            </div>
            <div class="timer-box">
              <div class="timer-label">Fin (calculado)</div>
              <div class="hm-inputs">
                <span class="hm-calc" id="irrig-off-${i}">${pad2(offH)}:${pad2(offM)}</span>
              </div>
            </div>
          </div>` : `
          <div class="irrig-param-row" style="justify-content:flex-start;gap:6px;font-size:12px;color:var(--text-3)">
            <span>Ciclos calculados:</span>
            <span style="color:var(--text-1);font-weight:600">${efectCiclos}</span>
          </div>`}
          <div class="irrig-param-row">
            <span class="timer-label">Pulso</span>
            <div class="irrig-num-unit">
              <input type="number" class="hm-input irrig-num" min="1" value="${pVal}"
                     onchange="setIrrigField(${i},'pulsoVal',+this.value)">
              <select class="cfg-select irrig-unit-sel" onchange="setIrrigField(${i},'pulsoUnit',+this.value)">
                <option value="1"    ${pUnit==1?   'selected':''}>seg</option>
                <option value="60"   ${pUnit==60?  'selected':''}>min</option>
                <option value="3600" ${pUnit==3600?'selected':''}>h</option>
              </select>
            </div>
          </div>
          <div class="irrig-duration${validErr ? ' err' : ''}">${validErr || durTxt + ' por ciclo'}</div>
          <div class="irrig-param-row">
            <span class="timer-label">Pausa</span>
            <div class="irrig-num-unit">
              <input type="number" class="hm-input irrig-num" min="0" value="${paVal}"
                     onchange="setIrrigField(${i},'pausaVal',+this.value)">
              <select class="cfg-select irrig-unit-sel" onchange="setIrrigField(${i},'pausaUnit',+this.value)">
                <option value="1"    ${paUnit==1?   'selected':''}>seg</option>
                <option value="60"   ${paUnit==60?  'selected':''}>min</option>
                <option value="3600" ${paUnit==3600?'selected':''}>h</option>
              </select>
            </div>
          </div>
          ${!tdEl ? `
          <div class="irrig-param-row">
            <span class="timer-label">Ciclos/dÃ­a</span>
            <input type="number" class="hm-input irrig-num" min="1" max="99" value="${cics}"
                   onchange="setIrrigField(${i},'ciclos',+this.value)">
          </div>` : ''}
          <div class="irrig-dias">
            <div class="timer-label" style="margin-top:8px;margin-bottom:4px">DÃ­as de riego</div>
            <div class="irrig-dias-grid">
              ${['D','L','M','X','J','V','S'].map((d, idx) => `
                <div class="irrig-dia">
                  <span>${d}</span>
                  <input type="checkbox" ${dias[idx] ? 'checked' : ''}
                         onchange="setIrrigDia(${i},${idx},this.checked)">
                </div>`).join('')}
            </div>
          </div>
        </div>`;
    } else if (activeMode === 'auto' && ILLUMINATION_RELAYS.has(i)) {
      const pad = n => String(n == null ? 0 : n).padStart(2, '0');
      const onClock = `${pad(cfg.horaOn)}:${pad(cfg.minOn)}`;
      const offClock = `${pad(cfg.horaOff)}:${pad(cfg.minOff)}`;
      extra = `
        <div class="mode-extra">
          <div class="timer-grid">
            <div class="timer-box">
              <div class="timer-label">Encendido</div>
              <input type="time" value="${escapeHtml(onClock)}"
                     onchange="setIllumTime(${i}, 'onTime', this.value)">
            </div>
            <div class="timer-box">
              <div class="timer-label">Apagado</div>
              <input type="time" value="${escapeHtml(offClock)}"
                     onchange="setIllumTime(${i}, 'offTime', this.value)">
            </div>
          </div>
        </div>`;
    } else if (activeMode === 'superciclo1313' && ILLUMINATION_RELAYS.has(i)) {
      const pad = n => String(n == null ? 0 : n).padStart(2, '0');
      const onH = cfg.horaOn ?? 0;
      const onM = cfg.minOn ?? 0;
      const offTotal = (onH * 60 + onM + 13 * 60) % 1440;
      const offH = cfg.horaOff ?? Math.floor(offTotal / 60);
      const offM = cfg.minOff ?? (offTotal % 60);
      const onClock = `${pad(onH)}:${pad(onM)}`;
      const offClock = `${pad(offH)}:${pad(offM)}`;
      extra = `
        <div class="mode-extra">
          <div class="timer-grid">
            <div class="timer-box">
              <div class="timer-label">Encendido</div>
              <input type="time" value="${escapeHtml(onClock)}"
                     onchange="setIllumTime(${i}, 'onTime', this.value)">
            </div>
            <div class="timer-box">
              <div class="timer-label">Apagado (13h)</div>
              <input type="time" value="${escapeHtml(offClock)}" disabled>
            </div>
          </div>
        </div>`;
    } else if (activeMode === 'ir') {
      const p = currentParams;
      extra = `
        <div class="mode-extra">
          <div class="mode-hint ir">
            â„ï¸ IR A/C: enciende frÃ­o si T &gt; <b>${p.tempAcOn}Â°C</b> Â· apaga si T &lt; <b>${p.tempAcOff}Â°C</b><br>
            <span style="opacity:.7">ConfigurÃ¡ los umbrales en la pestaÃ±a <b>ParÃ¡metros</b></span>
          </div>
        </div>`;
    } else if (activeMode === 'auto') {
      extra = `<div class="mode-extra"><div class="mode-hint">${escapeHtml(autoHintFor(i))}</div></div>`;
    } else if (activeMode === 'manual') {
      extra = `
        <div class="mode-extra">
          <div class="manual-actions">
            <button type="button" class="btn-on"  onclick="setManualState(${i}, true)">ENCENDER</button>
            <button type="button" class="btn-off" onclick="setManualState(${i}, false)">APAGAR</button>
          </div>
        </div>`;
    } else if (activeMode === 'timer') {
      extra = `
        <div class="mode-extra">
          <div class="timer-grid">
            <div class="timer-box">
              <div class="timer-label">Encendido</div>
              <input type="time" value="${escapeHtml(cfg.onTime || '06:00')}"
                     onchange="setTimerTime(${i}, 'onTime', this.value)">
            </div>
            <div class="timer-box">
              <div class="timer-label">Apagado</div>
              <input type="time" value="${escapeHtml(cfg.offTime || '22:00')}"
                     onchange="setTimerTime(${i}, 'offTime', this.value)">
            </div>
          </div>
        </div>`;
    }

    const on = !!r.on;
    cards.push(`
      <div class="relay-card ${on ? 'is-on' : ''}" data-relay-idx="${i}">
        <div class="relay-header">
          <div class="relay-left">
            <div class="relay-icon">${icon}</div>
            <div class="relay-text">
              <div class="relay-name">${escapeHtml(name)} <span style="opacity:.5;font-weight:500">Â· R${i}</span></div>
              <div class="relay-status">${escapeHtml(reasonForRelay({...r, idx:i, modeKind: cfg.mode, on}, currentParams))}</div>
            </div>
          </div>
          <span class="relay-state-dot ${on ? 'on' : ''}"></span>
        </div>
        <div class="mode-selector">${modeBtns}</div>
        ${extra}
        ${dirtyRelayIds.has(i) ? `<button type="button" class="save-appear" onclick="saveRelayToFirmware(${i}, 'Guardado')">ðŸ’¾ Guardar</button>` : ''}
      </div>
    `);
  }
  host.innerHTML = cards.join('');
  IRRIGATION_RELAYS.forEach(idx => updateIrrigOff(idx));
}

function updateSaveButton(i) {
  const card = document.querySelector(`.relay-card[data-relay-idx="${i}"]`);
  if (!card) return;
  let btn = card.querySelector('.save-appear');
  if (dirtyRelayIds.has(i)) {
    if (!btn) {
      btn = document.createElement('button');
      btn.type = 'button';
      btn.className = 'save-appear';
      btn.textContent = 'ðŸ’¾ Guardar';
      btn.onclick = () => saveRelayToFirmware(i, 'Guardado');
      card.appendChild(btn);
    }
  } else {
    if (btn) btn.remove();
  }
}

function autoHintFor(i) {
  const p = currentParams;
  if (p.acRelay      == i) return `EncenderÃ¡ cuando T > ${p.tempAcOn}Â°. Se apagarÃ¡ al llegar a ${p.tempAcOff}Â°.`;
  if (p.heaterRelay  == i) return `EncenderÃ¡ cuando T < ${p.tempHeaterOn}Â°. Se apagarÃ¡ al llegar a ${p.tempHeaterOff}Â°.`;
  if (p.humidRelay   == i) return `EncenderÃ¡ cuando HR < ${p.humHumidOn}%. Se apagarÃ¡ al llegar a ${p.humHumidOff}%.`;
  if (p.dehumidRelay == i) return `EncenderÃ¡ cuando HR > ${p.humDehumOn}%. Se apagarÃ¡ al llegar a ${p.humDehumOff}%.`;
  return `Sin asignaciÃ³n en ParÃ¡metros: este relÃ© no se activarÃ¡ en automÃ¡tico. Asignalo en la pestaÃ±a ParÃ¡metros.`;
}

// handlers Dispositivos
function setRelayMode(i, kind) {
  holdDevicesUi();
  const cfg = getRelayCfg(i);
  cfg.mode = kind;
  currentRelayCfg['r' + i] = cfg;
  dirtyRelayIds.add(i);
  saveRelayCfgLocal(currentRelayCfg);

  // ActualizaciÃ³n optimista del modeKind en lastState para que el badge del dashboard tambiÃ©n se actualice
  if (lastState?.relays?.['r' + i]) {
    lastState.relays['r' + i].modeKind = kind;
  }
  renderDevices();
  if (lastState) renderDashRelays(lastState);
}
function calcIrrigOff(cfg) {
  if (cfg.todoElDia) return { h: 0, m: 0 };
  const startSec = (cfg.horaOn ?? 8) * 3600 + (cfg.minOn ?? 0) * 60;
  const ciclos   = Math.max(1, cfg.ciclos    ?? 4);
  const pulsoSec = (cfg.pulsoVal ?? 30) * (cfg.pulsoUnit ?? 1);
  const pausaSec = (cfg.pausaVal ?? 10) * (cfg.pausaUnit ?? 60);
  const totalSec = ciclos * pulsoSec + Math.max(0, ciclos - 1) * pausaSec;
  const totalMin = Math.max(1, Math.ceil(totalSec / 60));
  const offSec   = (startSec + totalMin * 60) % 86400;
  return { h: Math.floor(offSec / 3600), m: Math.floor((offSec % 3600) / 60) };
}
function calcCiclosTodoElDia(cfg) {
  const pulsoSec = (cfg.pulsoVal ?? 30) * (cfg.pulsoUnit ?? 1);
  const pausaSec = (cfg.pausaVal ?? 10) * (cfg.pausaUnit ?? 60);
  const cicloSec = pulsoSec + pausaSec;
  return cicloSec > 0 ? Math.max(1, Math.floor(86400 / cicloSec)) : 1;
}
function irrigValidation(cfg) {
  const pulsoSec = (cfg.pulsoVal ?? 30) * (cfg.pulsoUnit ?? 1);
  if (pulsoSec <= 0) return 'El pulso debe ser mayor a 0';
  if (cfg.todoElDia) {
    const pausaSec = (cfg.pausaVal ?? 10) * (cfg.pausaUnit ?? 60);
    if (pausaSec <= 0) return 'La pausa debe ser mayor a 0 para "todo el dÃ­a"';
  }
  return null;
}
function formatDuration(sec) {
  if (!isFinite(sec) || sec <= 0) return '0s';
  const h = Math.floor(sec / 3600);
  const m = Math.floor((sec % 3600) / 60);
  const s = Math.floor(sec % 60);
  if (h > 0) return m > 0 ? `${h}h ${m}min` : `${h}h`;
  if (m > 0) return s > 0 ? `${m}min ${s}s` : `${m}min`;
  return `${s}s`;
}

function updateIrrigOff(i) {
  const el = document.getElementById('irrig-off-' + i);
  if (!el) return;
  const cfg = getRelayCfg(i);
  const { h, m } = calcIrrigOff(cfg);
  el.textContent = String(h).padStart(2,'0') + ':' + String(m).padStart(2,'0');
}

function applyClockTime(cfg, key, val, fallbackH, fallbackM) {
  const parts = parseHHMM(val, fallbackH, fallbackM);
  const clock = String(parts.h).padStart(2, '0') + ':' + String(parts.m).padStart(2, '0');
  if (key === 'onTime') {
    cfg.onTime = clock;
    cfg.horaOn = parts.h;
    cfg.minOn = parts.m;
  } else if (key === 'offTime') {
    cfg.offTime = clock;
    cfg.horaOff = parts.h;
    cfg.minOff = parts.m;
  }
}

function setIrrigField(i, key, val) {
  holdDevicesUi();
  const cfg = getRelayCfg(i);
  cfg[key] = val;
  currentRelayCfg['r' + i] = cfg;
  dirtyRelayIds.add(i);
  saveRelayCfgLocal(currentRelayCfg);
  updateIrrigOff(i);
  updateSaveButton(i);
}

function setIrrigDia(i, dayIdx, checked) {
  holdDevicesUi();
  const cfg = getRelayCfg(i);
  if (!Array.isArray(cfg.diasRiego)) cfg.diasRiego = [false,true,true,true,true,true,false];
  cfg.diasRiego[dayIdx] = checked;
  currentRelayCfg['r' + i] = cfg;
  dirtyRelayIds.add(i);
  saveRelayCfgLocal(currentRelayCfg);
  updateSaveButton(i);
}

function setIrrigTodoElDia(i, val) {
  holdDevicesUi();
  const cfg = getRelayCfg(i);
  cfg.todoElDia = !!val;
  currentRelayCfg['r' + i] = cfg;
  dirtyRelayIds.add(i);
  saveRelayCfgLocal(currentRelayCfg);
  renderDevices();
}
function setIrrigTime(i, val) {
  holdDevicesUi();
  const cfg = getRelayCfg(i);
  applyClockTime(cfg, 'onTime', val, 8, 0);
  currentRelayCfg['r' + i] = cfg;
  dirtyRelayIds.add(i);
  saveRelayCfgLocal(currentRelayCfg);
  updateIrrigOff(i);
  updateSaveButton(i);
}
function setIllumTime(i, key, val) {
  holdDevicesUi();
  const cfg = getRelayCfg(i);
  if (key === 'onTime') applyClockTime(cfg, key, val, 0, 0);
  else if (key === 'offTime') applyClockTime(cfg, key, val, 0, 0);
  else cfg[key] = val;
  currentRelayCfg['r' + i] = cfg;
  dirtyRelayIds.add(i);
  saveRelayCfgLocal(currentRelayCfg);
  updateSaveButton(i);
}
function setTimerTime(i, key, val) {
  holdDevicesUi();
  const cfg = getRelayCfg(i);
  if (key === 'onTime') applyClockTime(cfg, key, val, 6, 0);
  else if (key === 'offTime') applyClockTime(cfg, key, val, 22, 0);
  else cfg[key] = val;
  currentRelayCfg['r' + i] = cfg;
  dirtyRelayIds.add(i);
  saveRelayCfgLocal(currentRelayCfg);
  updateSaveButton(i);
}
function setManualState(i, on) {
  holdDevicesUi();
  const cfg = getRelayCfg(i);
  cfg.mode = 'manual';
  cfg.on = !!on;
  currentRelayCfg['r' + i] = cfg;
  dirtyRelayIds.delete(i);  // auto-save inmediato: no mostrar botÃ³n Guardar
  saveRelayCfgLocal(currentRelayCfg);

  setRelayVisualPending(i, on);
  if (!lastState) lastState = {};
  if (!lastState.relays) lastState.relays = {};
  if (!lastState.relays['r' + i]) lastState.relays['r' + i] = {};
  lastState.relays['r' + i].on = !!on;
  renderDashRelays(lastState);
  renderDevices();

  // Enviar al firmware y refrescar desde /api/state para confirmar con el ESP32.
  saveRelayToFirmware(i);
}

function sleep(ms) {
  return new Promise(resolve => setTimeout(resolve, ms));
}

function relayStateMatches(i, cfg, state) {
  const r = state && state.relays && state.relays['r' + i];
  if (!r) return false;
  const expectedMode = (AUTO_MANUAL_ONLY_RELAYS.has(i) && cfg.mode === 'timer') ? 'auto' : (cfg.mode || 'auto');
  const actualMode = modeFromRelayState(r);
  if (actualMode !== expectedMode) return false;
  if (expectedMode === 'manual' && !!r.on !== !!cfg.on) return false;
  return true;
}

async function waitForRelayConfirm(i, cfg, timeoutMs = 3500) {
  const start = Date.now();
  while (Date.now() - start < timeoutMs) {
    const rawState = await loadState();
    if (relayStateMatches(i, cfg, rawState)) return true;
    await sleep(300);
  }
  return false;
}

async function saveAllRelays() {
  const ids = Array.from(dirtyRelayIds);
  if (!ids.length) {
    showToast('No hay cambios para guardar');
    return;
  }
  showToast('Guardando dispositivosâ€¦');
  try {
    let allConfirmed = true;
    for (const i of ids) {
      const confirmed = await saveRelayToFirmware(i);
      if (!confirmed) allConfirmed = false;
    }
    showToast(allConfirmed ? 'Dispositivos guardados' : 'Algunos dispositivos quedaron pendientes', !allConfirmed);
  } catch {
    showToast('Guardado localmente (firmware sin /api/relay)', true);
  }
}

async function saveRelayToFirmware(i, okMsg) {
  const cfg = getRelayCfg(i);
  try {
    await postRelay(i);
    const confirmed = await waitForRelayConfirm(i, cfg);
    if (confirmed) {
      delete currentRelayCfg['r' + i];
      dirtyRelayIds.delete(i);
      updateSaveButton(i);
      if (okMsg) showToast(okMsg);
      await loadState();
      return true;
    } else {
      showToast('El ESP32 respondio, esperando estado real...', true);
      await loadState();
      return false;
    }
  } catch {
    showToast('Cambio local pendiente de guardar', true);
    return false;
  }
}

async function postRelay(i) {
  const cfg = getRelayCfg(i);
  const modeToSend = (AUTO_MANUAL_ONLY_RELAYS.has(i) && cfg.mode === 'timer') ? 'auto' : (cfg.mode || 'auto');
  let payloadOnTime = cfg.onTime || '';
  let payloadOffTime = cfg.offTime || '';
  let payloadHoraOff = cfg.horaOff ?? '';
  let payloadMinOff = cfg.minOff ?? '';
  if (IRRIGATION_RELAYS.has(i)) {
    const tdEl = !!cfg.todoElDia;
    const efHoraOn = tdEl ? 0 : (cfg.horaOn ?? 8);
    const efMinOn  = tdEl ? 0 : (cfg.minOn  ?? 0);
    const { h: offH, m: offM } = calcIrrigOff(cfg);
    payloadOnTime  = String(efHoraOn).padStart(2, '0') + ':' + String(efMinOn).padStart(2, '0');
    payloadOffTime = tdEl ? '00:00' : (String(offH).padStart(2, '0') + ':' + String(offM).padStart(2, '0'));
    payloadHoraOff = tdEl ? 0 : offH;
    payloadMinOff  = tdEl ? 0 : offM;
  } else if (ILLUMINATION_RELAYS.has(i) && (modeToSend === 'auto' || modeToSend === 'superciclo1313')) {
    payloadOnTime = String(cfg.horaOn ?? 0).padStart(2, '0') + ':' + String(cfg.minOn ?? 0).padStart(2, '0');
    if (modeToSend === 'superciclo1313') {
      const offTotal = ((cfg.horaOn ?? 0) * 60 + (cfg.minOn ?? 0) + 13 * 60) % 1440;
      payloadHoraOff = Math.floor(offTotal / 60);
      payloadMinOff = offTotal % 60;
    }
    payloadOffTime = String(payloadHoraOff || 0).padStart(2, '0') + ':' + String(payloadMinOff || 0).padStart(2, '0');
  }
  const body = new URLSearchParams({
    n: String(i),
    mode: modeToSend,
    modeNum: String(kindToNumericMode(modeToSend)),
    on: cfg.on ? '1' : '0',
    onTime: payloadOnTime,
    offTime: payloadOffTime,
    horaOn:  String(cfg.horaOn  ?? ''),
    minOn:   String(cfg.minOn   ?? ''),
    horaOff: String(payloadHoraOff),
    minOff:  String(payloadMinOff)
  });
  if (IRRIGATION_RELAYS.has(i)) {
    const tdEl = !!cfg.todoElDia;
    const efCiclos = tdEl ? calcCiclosTodoElDia(cfg) : (cfg.ciclos ?? 4);
    body.append('pulsoVal',  String(cfg.pulsoVal  ?? 30));
    body.append('pulsoUnit', String(cfg.pulsoUnit ?? 1));
    body.append('pausaVal',  String(cfg.pausaVal  ?? 10));
    body.append('pausaUnit', String(cfg.pausaUnit ?? 60));
    body.append('ciclos',    String(efCiclos));
    body.append('todoElDia', tdEl ? '1' : '0');
    body.append('horaOffCalc', String(payloadHoraOff));
    body.append('minOffCalc',  String(payloadMinOff));
    const dias = cfg.diasRiego || [false,true,true,true,true,true,false];
    dias.forEach((d, idx) => body.append('diaRiego' + idx, d ? '1' : '0'));
  }
  const res = await fetch('/api/relay', {
    method: 'POST',
    headers: { 'Content-Type': 'application/x-www-form-urlencoded', 'X-Requested-With': 'XMLHttpRequest' },
    body
  });
  if (!res.ok) throw new Error('relay save failed');
  return res;
}

// =========================
// Render PARÃMETROS
// =========================
function renderParams() {
  const p = currentParams;
  const tempVals = normalizeQuadValues('temp', [p.tempHeaterOn, p.tempHeaterOff, p.tempAcOff, p.tempAcOn]);
  const humVals  = normalizeQuadValues('hum',  [p.humHumidOn,  p.humHumidOff,  p.humDehumOff, p.humDehumOn]);
  QUAD_KEYS.temp.forEach((k, i) => { currentParams[k] = tempVals[i]; });
  QUAD_KEYS.hum.forEach((k, i) => { currentParams[k] = humVals[i]; });
  setQuadRange('temp', ...tempVals);
  setQuadRange('hum',  ...humVals);

  // selects de asignaciÃ³n: 6 relÃ©s + ninguno
  ['acRelay','heaterRelay','humidRelay','dehumidRelay'].forEach(key => {
    const sel = byId('a-' + key);
    if (!sel) return;
    const current = currentParams[key];
    const opts = ['<option value="0">â€” Ninguno â€”</option>'];
    for (let i = 1; i <= 8; i++) {
      const name = (lastState && lastState.relays && lastState.relays['r' + i] && lastState.relays['r' + i].name) || RELAY_DEFAULT_NAMES[i] || ('RelÃ© ' + i);
      opts.push(`<option value="${i}">R${i} Â· ${escapeHtml(name)}</option>`);
    }
    sel.innerHTML = opts.join('');
    sel.value = String(current == null ? 0 : current);
  });
}

function normalizeQuadValues(key, values) {
  const cfg = QUAD_CFG[key];
  const go = cfg.gapOuter, gi = cfg.gapInner;
  let [v1, v2, v3, v4] = values.map(v => parseFloat(v));
  if (![v1, v2, v3, v4].every(isFinite)) {
    [v1, v2, v3, v4] = key === 'temp' ? [18, 22, 25, 28] : [40, 55, 65, 75];
  }
  v1 = Math.max(cfg.min, Math.min(cfg.max, v1));
  v4 = Math.max(cfg.min, Math.min(cfg.max, v4));
  v2 = Math.max(v1 + go, Math.min(v3 - gi, v2));
  v3 = Math.max(v2 + gi, Math.min(v4 - go, v3));
  v2 = Math.max(cfg.min, Math.min(cfg.max, v2));
  v3 = Math.max(cfg.min, Math.min(cfg.max, v3));
  return [v1, v2, v3, v4];
}

function setQuadRange(key, v1, v2, v3, v4) {
  [v1, v2, v3, v4].forEach((val, i) => {
    const el = byId('qr-' + key + '-' + (i + 1));
    if (el) el.value = val;
  });
  updateQuadFill(key, v1, v2, v3, v4);
  updateQuadLabels(key, v1, v2, v3, v4);
}

function onQuadRange(key, which) {
  for (let i = 1; i <= 4; i++) {
    const el = byId('qr-' + key + '-' + i);
    const thumb = byId('qt-' + key + '-' + i);
    if (thumb) thumb.classList.toggle('is-active', i === which);
  }
  const cfg = QUAD_CFG[key];
  const go = cfg.gapOuter, gi = cfg.gapInner;
  const vals = [1,2,3,4].map(i => parseFloat(byId('qr-' + key + '-' + i).value));
  let [v1, v2, v3, v4] = vals;

  if (which === 1) v1 = Math.min(v1, v2 - go);
  if (which === 2) { v2 = Math.max(v2, v1 + go); v2 = Math.min(v2, v3 - gi); }
  if (which === 3) { v3 = Math.max(v3, v2 + gi); v3 = Math.min(v3, v4 - go); }
  if (which === 4) v4 = Math.max(v4, v3 + go);

  v1 = Math.max(cfg.min, v1);
  v4 = Math.min(cfg.max, v4);
  v2 = Math.max(v1 + go, Math.min(v3 - gi, v2));
  v3 = Math.max(v2 + gi, Math.min(v4 - go, v3));

  [v1, v2, v3, v4].forEach((val, i) => {
    const el = byId('qr-' + key + '-' + (i + 1));
    if (el) el.value = val;
    currentParams[QUAD_KEYS[key][i]] = val;
  });
  dirtyParams = true;
  saveParamsLocal(currentParams);
  updateQuadFill(key, v1, v2, v3, v4);
  updateQuadLabels(key, v1, v2, v3, v4);
}

function updateQuadFill(key, v1, v2, v3, v4) {
  const cfg = QUAD_CFG[key];
  const span = cfg.max - cfg.min;
  const pct = v => (v - cfg.min) / span * 100;
  const cold    = byId('qf-' + key + '-cold');
  const comfort = byId('qf-' + key + '-comfort');
  const hot     = byId('qf-' + key + '-hot');
  if (cold) { cold.style.left = pct(v1) + '%'; cold.style.width = (pct(v2) - pct(v1)) + '%'; }
  if (comfort) { comfort.style.left = pct(v2) + '%'; comfort.style.width = (pct(v3) - pct(v2)) + '%'; }
  if (hot)  { hot.style.left = pct(v3) + '%'; hot.style.width = (pct(v4) - pct(v3)) + '%'; }
}

function applySetpoint(key) {
  const el = byId('sp-' + key);
  if (!el) return;
  let S = parseFloat(el.value);
  if (!isFinite(S)) return;
  const { min: absMin, max: absMax, step, gapOuter: go, gapInner: gi } = QUAD_CFG[key];
  const roundToStep = v => Math.round(v / step) * step;
  S = Math.max(absMin, Math.min(absMax, roundToStep(S)));
  const halfInner = Math.ceil((gi / 2) / step) * step;
  let v2 = roundToStep(S - halfInner);
  let v3 = roundToStep(S + halfInner);
  let v1 = v2 - go;
  let v4 = v3 + go;
  // clamp to absolute range
  v1 = Math.max(absMin, v1);
  v4 = Math.min(absMax, v4);
  v2 = Math.max(v1 + go, Math.min(v3 - gi, v2));
  v3 = Math.max(v2 + gi, Math.min(v4 - go, v3));
  QUAD_KEYS[key].forEach((k, i) => { currentParams[k] = [v1, v2, v3, v4][i]; });
  dirtyParams = true;
  saveParamsLocal(currentParams);
  setQuadRange(key, v1, v2, v3, v4);
}

function updateQuadLabels(key, v1, v2, v3, v4) {
  const fmt = QUAD_CFG[key].fmt;
  [v1, v2, v3, v4].forEach((val, i) => {
    const el = byId('qv-' + key + '-' + (i + 1));
    if (el) el.textContent = fmt(val);
  });
  updateQuadScale(key, v1, v2, v3, v4);
}

function updateQuadScale(key, v1, v2, v3, v4) {
  const cfg = QUAD_CFG[key];
  const span = cfg.max - cfg.min;
  const pct = v => (v - cfg.min) / span * 100;
  const pointText = v => key === 'temp' ? v.toFixed(1) : String(Math.round(v));
  [v1, v2, v3, v4].forEach((val, i) => {
    const el = byId('qp-' + key + '-' + (i + 1));
    const thumb = byId('qt-' + key + '-' + (i + 1));
    const left = pct(val) + '%';
    if (el) {
      el.style.left = left;
      el.textContent = pointText(val);
    }
    if (thumb) thumb.style.left = left;
  });

  const center = (v2 + v3) / 2;
  const marker = byId('qc-' + key);
  const markerValue = byId('qcv-' + key);
  const setpoint = byId('sp-' + key);
  const icon = key === 'temp' ? 'ðŸŒ¡ï¸' : 'ðŸ’§';
  if (marker) marker.style.left = pct(center) + '%';
  if (markerValue) markerValue.innerHTML = `<span class="quad-setpoint-icon">${icon}</span>${pointText(center)}`;
  if (setpoint) setpoint.value = key === 'temp' ? center.toFixed(1) : String(Math.round(center));
}

function formatParam(key, val) {
  const v = parseFloat(val);
  if (!isFinite(v)) return '--';
  if (key === 'tempMin' || key === 'tempMax') return v.toFixed(1) + ' Â°C';
  return Math.round(v) + ' %';
}

function onAssignChange(key, val) {
  currentParams[key] = parseInt(val, 10) || 0;
  dirtyParams = true;
  saveParamsLocal(currentParams);
}

async function saveParams() {
  showToast('Guardando parÃ¡metrosâ€¦');
  try {
    const body = new URLSearchParams(Object.entries(currentParams).reduce((acc, [k, v]) => {
      acc[k] = String(v);
      return acc;
    }, {}));
    const res = await fetch('/api/params', {
      method: 'POST',
      headers: { 'Content-Type': 'application/x-www-form-urlencoded', 'X-Requested-With': 'XMLHttpRequest' },
      body
    });
    if (!res.ok) throw new Error();
    showToast('ParÃ¡metros guardados');
    dirtyParams = false;
    await loadState();
    renderParams();
  } catch {
    showToast('Guardado localmente (firmware sin /api/params)', true);
  }
}

// =========================
// HistÃ³rico 24h (localStorage)
// =========================
const HIST_KEY = 'druida_history_v1';
const HIST_HOURS = 24;
const SAMPLE_INTERVAL_MS = 60 * 1000;

function loadHistory() {
  try {
    const raw = localStorage.getItem(HIST_KEY);
    if (!raw) return [];
    const arr = JSON.parse(raw);
    if (!Array.isArray(arr)) return [];
    const cutoff = Date.now() - HIST_HOURS * 3600 * 1000;
    return arr.filter(p => p && p.t >= cutoff);
  } catch { return []; }
}
function saveHistory(arr) {
  try { localStorage.setItem(HIST_KEY, JSON.stringify(arr)); } catch {}
}
function pushHistory(temp, hum) {
  const t = parseFloat(temp), h = parseFloat(hum);
  if (!isFinite(t) && !isFinite(h)) return;
  const arr = loadHistory();
  const last = arr[arr.length - 1];
  if (last && Date.now() - last.t < SAMPLE_INTERVAL_MS) return;
  arr.push({ t: Date.now(), temp: isFinite(t) ? t : null, hum: isFinite(h) ? h : null });
  saveHistory(arr);
}
function fmtTime(ts) {
  const d = new Date(ts);
  const pad = n => String(n).padStart(2, '0');
  return pad(d.getHours()) + ':' + pad(d.getMinutes());
}
function computeExtremes(arr, key) {
  let min = null, max = null, minTs = null, maxTs = null;
  arr.forEach(p => {
    const v = p[key];
    if (v == null || !isFinite(v)) return;
    if (min == null || v < min) { min = v; minTs = p.t; }
    if (max == null || v > max) { max = v; maxTs = p.t; }
  });
  return { min, max, minTs, maxTs };
}
function updateExtremes() {
  const arr = loadHistory();
  const t = computeExtremes(arr, 'temp');
  const h = computeExtremes(arr, 'hum');
  setText('tempMin', t.min != null ? t.min.toFixed(1) + 'Â°' : '--');
  setText('tempMax', t.max != null ? t.max.toFixed(1) + 'Â°' : '--');
  setText('tempMinHora', t.minTs ? fmtTime(t.minTs) : '--:--');
  setText('tempMaxHora', t.maxTs ? fmtTime(t.maxTs) : '--:--');
  setText('humMin', h.min != null ? Math.round(h.min) + '%' : '--');
  setText('humMax', h.max != null ? Math.round(h.max) + '%' : '--');
  setText('humMinHora', h.minTs ? fmtTime(h.minTs) : '--:--');
  setText('humMaxHora', h.maxTs ? fmtTime(h.maxTs) : '--:--');
}

// =========================
// SVG charts (24h history)
// =========================
function drawSvgChart(svgId, emptyId, lastId, startId, endId, countId, points, key, unit, color, decimals) {
  const svg = byId(svgId);
  const emptyEl = byId(emptyId);
  if (!svg) return;
  const validPts = points.filter(p => p[key] != null && isFinite(p[key]));
  if (validPts.length < 2) {
    svg.innerHTML = '';
    if (emptyEl) emptyEl.classList.remove('hidden');
    if (lastId)  setText(lastId, '--');
    if (startId) setText(startId, '--:--');
    if (endId)   setText(endId, '--:--');
    if (countId) setText(countId, 'Sin muestras');
    return;
  }
  if (emptyEl) emptyEl.classList.add('hidden');
  const dec = decimals ?? 1;
  const last = validPts[validPts.length - 1];
  if (lastId)  setText(lastId, last[key].toFixed(dec) + unit);
  if (startId) setText(startId, fmtTime(validPts[0].t));
  if (endId)   setText(endId,   fmtTime(last.t));
  if (countId) setText(countId, validPts.length + ' muestras');

  const W = 320, H = 118;
  const padL = 4, padR = 4, padT = 14, padB = 4;
  const w = W - padL - padR, h = H - padT - padB;
  const vals  = validPts.map(p => p[key]);
  const minV  = Math.min(...vals), maxV = Math.max(...vals);
  const range = maxV - minV || 1;
  const tFirst = validPts[0].t, tLast = last.t;
  const tRange = tLast - tFirst || 1;
  const px = t  => padL + ((t - tFirst) / tRange) * w;
  const py = v  => padT + h - ((v - minV) / range) * h;
  const ptStr = validPts.map(p => `${px(p.t).toFixed(1)},${py(p[key]).toFixed(1)}`).join(' ');
  const topLbl = key === 'hum' ? Math.round(maxV) + unit : maxV.toFixed(dec) + unit;
  const botLbl = key === 'hum' ? Math.round(minV) + unit : minV.toFixed(dec) + unit;
  svg.innerHTML = `
    <defs>
      <filter id="glow-${svgId}" x="-20%" y="-20%" width="140%" height="140%">
        <feGaussianBlur stdDeviation="2.5" result="blur"/>
        <feMerge><feMergeNode in="blur"/><feMergeNode in="SourceGraphic"/></feMerge>
      </filter>
    </defs>
    <text class="history-scale" x="${padL + 2}" y="${padT - 2}" text-anchor="start">${topLbl}</text>
    <text class="history-scale" x="${padL + 2}" y="${H - 1}" text-anchor="start">${botLbl}</text>
    <polyline class="history-line-glow" points="${ptStr}" stroke="${color}" filter="url(#glow-${svgId})"/>
    <polyline class="history-line" points="${ptStr}" stroke="${color}"/>
    <circle class="history-dot" cx="${px(last.t).toFixed(1)}" cy="${py(last[key]).toFixed(1)}" r="3.5" fill="${color}"/>`;
}

function drawCharts() {
  const arr = loadHistory();
  drawSvgChart('svgTemp', 'histTempEmpty', 'histTempLast', 'histTempStart', 'histTempEnd', 'histTempCount',
    arr, 'temp', 'Â°', '#f87171', 1);
  drawSvgChart('svgHum', 'histHumEmpty', 'histHumLast', 'histHumStart', 'histHumEnd', 'histHumCount',
    arr, 'hum', '%', '#60a5fa', 0);
}

// =========================
// VPD / online / utils
// =========================
function updateVpdBadge(vpdRaw) {
  const el = byId('vpdBadge');
  if (!el) return;
  const v = parseFloat(String(vpdRaw).replace(',', '.'));
  if (!isFinite(v)) { el.textContent = '--'; el.className = 'vpd-badge'; return; }
  let label = 'Ã“ptimo', cls = 'optimal';
  if (v < 0.4)       { label = 'Muy bajo'; cls = 'low'; }
  else if (v < 0.8)  { label = 'Bajo';     cls = 'moderate'; }
  else if (v <= 1.2) { label = 'Ã“ptimo';   cls = 'optimal'; }
  else if (v <= 1.6) { label = 'Moderado'; cls = 'moderate'; }
  else               { label = 'Alto';     cls = 'high'; }
  el.textContent = label;
  el.className = 'vpd-badge ' + cls;
}
function setOnline(ok) {
  const b = byId('onlineBadge'); const l = byId('onlineLabel');
  if (!b) return;
  b.classList.toggle('offline', !ok);
  if (l) l.textContent = ok ? 'En lÃ­nea' : 'Sin conexiÃ³n';
}
function setText(id, val) { const el = byId(id); if (el) el.textContent = val; }
function setInputValue(id, val, force = false) {
  const el = byId(id);
  if (!el) return;
  if (!force && el.dataset.userEdited === '1') return;
  el.value = val || '';
}
function getLocalFechaHora() {
  const d   = new Date();
  const pad = n => String(n).padStart(2, '0');
  return `${d.getDate()}/${d.getMonth()+1}/${d.getFullYear()} ${pad(d.getHours())}:${pad(d.getMinutes())}:${pad(d.getSeconds())}`;
}
function displayFechaHora(state) {
  if (!state) return '--/--/---- --:--:--';
  if (state.fechaHora && state.fechaHora !== '--/--/---- --:--:--') return state.fechaHora;
  const rtc = state.rtc || {};
  const rtcDate = rtc.date || state.rtcDate || '';
  const rtcTime = rtc.time || state.rtcTime || '';
  if (rtcDate && rtcTime && rtcDate !== '--/--/----' && rtcTime !== '--:--:--') return `${rtcDate} ${rtcTime}`;
  if (rtcTime && rtcTime !== '--:--:--' && rtcTime !== '--:--') return `${rtcDate || '--/--/----'} ${rtcTime}`;
  return '--/--/---- --:--:--';
}
function rtcSummary(state) {
  if (!state) return '--';
  const rtc = state.rtc || {};
  const time = rtc.time || state.rtcTime || '';
  if (!time || time === '--:--:--') return '--';
  if (rtc.valid === false || state.rtcValid === false) return `${time.slice(0, 5)} (sin pila)`;
  return time.slice(0, 5);
}
function maskedLen(len) {
  const n = Number(len);
  if (!Number.isFinite(n) || n <= 0) return '';
  return 'â€¢'.repeat(Math.min(n, 64));
}
function wifiPasswordFromState(state) {
  if (!state) return '';
  return state.password ?? state.wifiPass ?? '';
}
function hydrateWifiFields(state, force = false) {
  if (!state) return;
  const ssid = state.ssid ?? state.wifiSsid ?? '';
  const pass = wifiPasswordFromState(state);
  const passLen = pass ? pass.length : Number(state.wifiPassLength || 0);
  const hasWifiUser = 'wifiUser' in state;
  const user = state.wifiUser ?? '';
  setInputValue('wifiSsid', ssid, force);
  // Solo actualizar el campo password si tenemos la contraseña real.
  // Si pass está vacío pero passLen > 0, la API aún no nos dio el valor —
  // no limpiar lo que el usuario ya tiene cargado.
  if (pass) {
    setInputValue('wifiPass', pass, force);
  } else if (passLen === 0 && force) {
    setInputValue('wifiPass', '', true);
  }
  if (hasWifiUser && (user || force)) setInputValue('wifiUser', user, force);
  const cb = byId('wifiWithUser');
  if (cb && hasWifiUser && (user || force)) {
    cb.checked = user.length > 0;
    toggleWifiUser(cb.checked);
  }
  const hint = byId('wifiPassHint');
  if (hint) {
    const mask = pass ? 'â€¢'.repeat(Math.min(pass.length, 64)) : maskedLen(passLen);
    hint.textContent = mask
      ? `Guardada: ${mask} (${passLen || pass.length} caracteres)`
      : 'Sin contraseÃ±a guardada';
  }
  const status = byId('wifiStatusText');
  if (status) status.textContent = ssid
    ? 'Red WiFi actual del dispositivo. GuardÃ¡ para cambiarla.'
    : 'Sin red guardada. BuscÃ¡ una red o escribÃ­ el SSID manualmente.';
}

function toggleWifiUser(show) {
  const box = byId('wifiUserBox');
  if (box) box.style.display = show ? '' : 'none';
}
function formatNum(raw, suffix, decimals) {
  if (raw == null || raw === '' || raw === '--') return '--';
  const s = String(raw);
  const n = parseFloat(s.replace(',', '.'));
  if (!isFinite(n)) return s;
  const d = decimals != null ? decimals : 1;
  return n.toFixed(d) + (suffix || '');
}

// =========================
// loadState
// =========================
async function loadState() {
  try {
    const res = await fetch('/api/state');
    if (!res.ok) throw new Error('state-' + res.status);
    // El ESP32 puede enviar NaN o Infinity â€” no son JSON vÃ¡lido, los sanitizamos
    const raw = await res.text();
    const s   = JSON.parse(raw.replace(/:[ \t]*NaN\b/g, ': null').replace(/:[ \t]*[-]?Infinity\b/g, ': null'));
    lastState = mergeIncomingState(s);
    const state = lastState;

    setText('temp',      formatNum(state.temp, 'Â°'));
    setText('hum',       formatNum(state.hum, '%', 0));
    setText('dpv',       formatNum(state.dpv, ' kPa'));
    setText('fechaHora', displayFechaHora(state));
    setText('pRtc', rtcSummary(state));
    hydrateWifiFields(state);

    if (state.diaVege  !== undefined) setText('diaVege',  'DIA VEGE: '   + state.diaVege);
    if (state.diaFlora !== undefined) setText('diaFlora', 'DIA FLORA: '  + state.diaFlora);
    if (state.diaSuper !== undefined) {
      setText('diaSuper', 'SUPERCICLOS: ' + state.diaSuper);
      const el = byId('diaSuper'); if (el) el.classList.remove('hidden');
    }

    ingestServerState(state);
    updateVpdBadge(state.dpv);
    renderDashRelays(state);
    if (devicesViewIsOpen()) {
      if (devicesUiIsBusy()) refreshDeviceRelayVisuals(state);
      else renderDevices();
    }

    setOnline(true);
    return s;
  } catch {
    // Fallback: si /api/state falla, intentar mostrar datos desde /api/sensors
    setOnline(await fetchSensorFallback());
    if (devicesViewIsOpen() && !devicesUiIsBusy()) renderDevices();
    return null;
  }
}

// Fallback: muestra datos del sensor ambiente en el dashboard cuando /api/state falla
async function fetchSensorFallback() {
  try {
    const res = await fetch('/api/sensors');
    if (!res.ok) return false;
    const raw = await res.text();
    const d   = JSON.parse(raw.replace(/:[ \t]*NaN\b/g, ': null').replace(/:[ \t]*[-]?Infinity\b/g, ': null'));
    if (!d || !Array.isArray(d.sensors)) return false;
    // Buscar el primer sensor ambiente habilitado con datos
    const ambient = d.sensors.find(s => s.enabled !== false && s.type !== 'soil' && s.temp != null);
    if (!ambient) return false;
    setText('temp', formatNum(ambient.temp, 'Â°'));
    if (ambient.hum != null) setText('hum', formatNum(ambient.hum, '%', 0));
    if (ambient.dpv != null) {
      setText('dpv', formatNum(ambient.dpv, ' kPa'));
      updateVpdBadge(ambient.dpv);
    }
    // Mostrar la hora local del navegador como referencia
    setText('fechaHora', getLocalFechaHora());

    pushHistory(ambient.temp, ambient.hum);
    updateExtremes();
    drawCharts();
    return true; // hay datos â†’ mostrar "En lÃ­nea"
  } catch {
    return false;
  }
}

// =========================
// Panel views
// =========================
const ALL_VIEWS = [
  'homeView','devicesView','paramsView',
  'panelView','sensorsView','sensorConfigView','wifiView','alertsView','updatesView'
];

function setViewEx(id) {
  ALL_VIEWS.forEach(v => {
    const el = byId(v);
    if (el) el.classList.add('preloaded-hidden');
  });
  const target = byId(id);
  if (target) target.classList.remove('preloaded-hidden');

  const tabMap = { homeView:'homeView', devicesView:'devicesView', paramsView:'paramsView' };
  document.querySelectorAll('.tab').forEach(t => {
    t.classList.toggle('active', t.dataset.tab === (tabMap[id] || '__none__'));
  });
  window.scrollTo({ top: 0, behavior: 'instant' in window ? 'instant' : 'auto' });
}

// Patch setView to use extended list
const _setViewOrig = setView;
// Override global setView so existing calls still work
function setView(id) {
  ALL_VIEWS.forEach(v => {
    const el = byId(v);
    if (el) el.classList.add('preloaded-hidden');
  });
  const target = byId(id);
  if (target) target.classList.remove('preloaded-hidden');
  const tabMap = { homeView:'homeView', devicesView:'devicesView', paramsView:'paramsView' };
  document.querySelectorAll('.tab').forEach(t => {
    t.classList.toggle('active', t.dataset.tab === (tabMap[id] || '__none__'));
  });
  window.scrollTo({ top: 0, behavior: 'instant' in window ? 'instant' : 'auto' });
}

function showPreloadedPanel()  { setView('panelView');  loadInfo(); }
function showPreloadedSensors(){ setView('sensorsView'); loadSensors(); }
function showPreloadedWifi()   { setView('wifiView'); loadWifiPanel(); }
function showPreloadedAlerts() { setView('alertsView'); loadAlerts(); }
function showPreloadedUpdates() {
  setView('updatesView');
}
// =========================
// Info del dispositivo (/api/info)
// =========================
async function loadInfo() {
  try {
    const r = await fetch('/api/info');
    if (!r.ok) throw new Error();
    const d = await r.json();
    setText('pVersion', d.version || '--');
    setText('pPlan',    d.plan    || '--');
    setText('pUptime',  d.uptime  || '--');
    setText('pWifiSsid', d.ssid   || '--');
  } catch {
    setText('pVersion', '3.0');
    setText('pPlan',    'EstÃ¡ndar');
    setText('pUptime',  '--');
  }
}

// =========================
// Sensores (/api/sensors)
// =========================
const SENSOR_CFG_KEY = 'druida_sensors_v2';

const SENSOR_DEFAULTS = [
  { id:'s1', name:'Ambiente 1', type:'ambient', addr:'1', enabled:true  },
  { id:'s2', name:'Ambiente 2', type:'ambient', addr:'2', enabled:false },
  { id:'s3', name:'Ambiente 3', type:'ambient', addr:'3', enabled:false },
  { id:'s4', name:'Ambiente 4', type:'ambient', addr:'4', enabled:false },
  { id:'s5', name:'Suelo 1',    type:'soil',    addr:'5', enabled:false },
  { id:'s6', name:'Suelo 2',    type:'soil',    addr:'6', enabled:false },
];

function loadSensorCfg() {
  try {
    const raw = localStorage.getItem(SENSOR_CFG_KEY);
    if (raw) {
      const saved = JSON.parse(raw);
      return SENSOR_DEFAULTS.map(d => Object.assign({}, d, saved.find(s => s.id === d.id) || {}));
    }
  } catch {}
  return SENSOR_DEFAULTS.map(d => Object.assign({}, d));
}
function saveSensorCfgLocal(arr) {
  try { localStorage.setItem(SENSOR_CFG_KEY, JSON.stringify(arr)); } catch {}
}

let currentSensors = loadSensorCfg();
let editingSensorId = null;

async function loadSensors() {
  try {
    const r = await fetch('/api/sensors');
    if (!r.ok) throw new Error();
    const d = await r.json();
    if (d.sensors && Array.isArray(d.sensors)) {
      // merge server data into local cfg
      currentSensors = currentSensors.map(cfg => {
        const srv = d.sensors.find(s => s.id === cfg.id || s.addr == cfg.addr);
        if (!srv) return cfg;
        return Object.assign({}, cfg, {
          enabled: srv.enabled !== undefined ? !!srv.enabled : cfg.enabled,
          connected: srv.connected,
          temp: numberOrNull(srv.temp),
          hum: numberOrNull(srv.hum),
          dpv: numberOrNull(srv.dpv),
          ec: numberOrNull(srv.ec),
          moisture: numberOrNull(srv.moisture ?? srv.hum),
          addr: srv.addr != null ? String(srv.addr) : cfg.addr,
          type: srv.type || cfg.type,
          name: srv.name || cfg.name
        });
      });
    }
  } catch { /* sin /api/sensors â€” mostrar config local */ }
  renderSensors();
}

function renderSensors() {
  const ambient = currentSensors.filter(s => s.type === 'ambient' && s.enabled);
  const soil    = currentSensors.filter(s => s.type === 'soil'    && s.enabled);
  const canAddAmbient = ambient.length < 4;
  const canAddSoil    = soil.length < 2;

  byId('sensorsAmbient').innerHTML =
    (ambient.length ? ambient.map(sensorCardHtml).join('') : `<div class="sensor-empty">Sensor ambiente ID 1 incluido de fabrica.</div>`) +
    (canAddAmbient ? `<button class="add-sensor-btn" onclick="addSensor('ambient')">+ Agregar sensor de ambiente</button>` : '');

  byId('sensorsSoil').innerHTML =
    (soil.length ? soil.map(sensorCardHtml).join('') : `<div class="sensor-empty">No hay sensores de suelo agregados.</div>`) +
    (canAddSoil ? `<button class="add-sensor-btn" onclick="addSensor('soil')">+ Agregar sensor de suelo</button>` : '');
}

function addSensor(type) {
  const next = currentSensors.find(s => s.type === type && !s.enabled);
  if (!next) return;
  next.enabled = true;
  saveSensorCfgLocal(currentSensors);
  showSensorConfig(next.id);
}

function removeSensor(id) {
  const s = currentSensors.find(x => x.id === id);
  if (!s) return;
  if (s.id === 's1') {
    showToast('El sensor ambiente 1 viene de fabrica');
    return;
  }
  s.enabled = false;
  saveSensorCfgLocal(currentSensors);
  saveSensorEnabledToDevice(s, false);
  showToast('Sensor eliminado');
  showPreloadedSensors();
}

function sensorCardHtml(s) {
  const conn = s.connected === true;
  const conn_unknown = s.connected === undefined;
  const isOn = conn || conn_unknown;

  let reading = '';
  if (s.type === 'ambient') {
    if (s.temp != null) reading += s.temp.toFixed(1) + 'Â°C';
    if (s.hum  != null) reading += (reading ? '<br>' : '') + Math.round(s.hum) + '% HR';
    if (s.dpv  != null) reading += (reading ? '<br>' : '') + 'DPV ' + s.dpv.toFixed(1);
  } else {
    if (s.moisture != null) reading += 'Hum: ' + Math.round(s.moisture) + '%';
    if (s.ec != null)       reading += (reading ? '<br>' : '') + 'EC ' + Math.round(s.ec);
    if (s.temp != null)     reading += (reading ? '<br>' : '') + s.temp.toFixed(1) + 'Â°C';
  }
  if (!reading) reading = conn_unknown ? '<span style="color:var(--text-3)">sin datos</span>' : '';

  const badge = conn_unknown
    ? `<span class="sensor-badge" style="background:var(--amber-bg);color:var(--amber-text);border-color:rgba(245,158,11,.2)">Sin verificar</span>`
    : conn
      ? `<span class="sensor-badge">Conectado</span>`
      : `<span class="sensor-badge off">Sin seÃ±al</span>`;

  return `
    <div class="sensor-card ${conn ? 'connected' : ''}" onclick="showSensorConfig('${s.id}')">
      <span class="sensor-dot ${isOn ? 'on' : ''}"></span>
      <div class="sensor-info">
        <div class="sensor-name">${escapeHtml(s.name)}</div>
        <div class="sensor-meta">DirecciÃ³n: ${escapeHtml(s.addr)} Â· ${s.type === 'ambient' ? 'Ambiente' : 'Suelo'}</div>
      </div>
      <div style="display:flex;flex-direction:column;align-items:flex-end;gap:5px">
        ${badge}
        ${reading ? `<div class="sensor-reading">${reading}</div>` : ''}
      </div>
    </div>`;
}

function showSensorConfig(id) {
  editingSensorId = id;
  const s = currentSensors.find(x => x.id === id);
  if (!s) return;
  const card = byId('sensorConfigCard');
  if (!card) return;
  card.innerHTML = `
    <div class="card-title-row">
      <span>${s.type === 'ambient' ? 'ðŸŒ¡ï¸' : 'ðŸŒ±'}</span>
      <div class="card-title">Configurar sensor</div>
    </div>
    <label class="cfg-label">Nombre</label>
    <input type="text" id="scName" class="cfg-input" value="${escapeHtml(s.name)}">
    <label class="cfg-label">DirecciÃ³n / ID</label>
    <input type="number" id="scAddr" class="cfg-input" value="${escapeHtml(s.addr)}" min="${s.type === 'ambient' ? 1 : 5}" max="${s.type === 'ambient' ? 4 : 6}" step="1"
           placeholder="Ej: 1  Â·  0x40  Â·  28FF3A...">
    <div class="param-hint" style="margin-top:6px">
      UsÃ¡ el mismo identificador que configuraste en el firmware
      (nÃºmero Modbus, direcciÃ³n IÂ²C en hex, o ROM de sensor 1-Wire).
    </div>
    <label class="cfg-label">Tipo</label>
    <select id="scType" class="cfg-select" disabled>
      <option value="ambient" ${s.type==='ambient'?'selected':''}>Ambiente (TÂ° y HR)</option>
      <option value="soil"    ${s.type==='soil'   ?'selected':''}>Suelo (humedad)</option>
    </select>
    ${s.id === 's1' ? '' : `<button type="button" class="btn-remove-sensor" onclick="removeSensor('${s.id}')">Eliminar sensor</button>`}`;
  setView('sensorConfigView');
}

function saveSensorConfig() {
  const s = currentSensors.find(x => x.id === editingSensorId);
  if (!s) return;
  s.name = (byId('scName').value || s.name).trim();
  s.type = byId('scType').value;
  const addr = normalizeSensorAddress(s.type, byId('scAddr').value || s.addr);
  if (!addr) {
    showToast(s.type === 'ambient' ? 'ID ambiente valido: 1 a 4' : 'ID suelo valido: 5 o 6', true);
    return;
  }
  s.addr = String(addr);
  s.enabled = true;
  saveSensorCfgLocal(currentSensors);
  const body = new URLSearchParams({ id: s.id, name: s.name, addr: s.addr, sensor: s.addr, type: s.type, enabled: '1' });
  fetch('/api/sensor', {
    method:'POST',
    headers:{'Content-Type':'application/x-www-form-urlencoded','X-Requested-With':'XMLHttpRequest'},
    body
  }).catch(() => {});
  showToast('Sensor guardado');
  showPreloadedSensors();
}

function numberOrNull(v) {
  if (v === null || v === undefined || v === '--') return null;
  const n = Number(v);
  return Number.isFinite(n) ? n : null;
}

function normalizeSensorAddress(type, raw) {
  const n = parseInt(String(raw).replace(/[^\d]/g, ''), 10);
  if (type === 'ambient') return n >= 1 && n <= 4 ? n : 0;
  return n >= 5 && n <= 6 ? n : 0;
}

function saveSensorEnabledToDevice(s, enabled) {
  const body = new URLSearchParams({ sensor: s.addr, enabled: enabled ? '1' : '0' });
  fetch('/api/sensor', {
    method:'POST',
    headers:{'Content-Type':'application/x-www-form-urlencoded','X-Requested-With':'XMLHttpRequest'},
    body
  }).catch(() => {});
}

// =========================
// WiFi (panel)
// =========================
async function loadWifiPanel() {
  try {
    const r = await fetch('/api/state');
    if (r.ok) {
      const raw = await r.text();
      const s = JSON.parse(raw.replace(/:[ \t]*NaN\b/g, ': null').replace(/:[ \t]*[-]?Infinity\b/g, ': null'));
      hydrateWifiFields(s, true);
      return;
    }
  } catch {}

  try {
    const r = await fetch('/api/info');
    if (!r.ok) return;
    const d = await r.json();
    hydrateWifiFields(d, true);
  } catch {}
}

function selectScannedWifi() {
  const sel = byId('wifiScanSel');
  const input = byId('wifiSsid');
  if (!sel || !input || !sel.value) return;
  input.value = sel.value;
  input.dataset.userEdited = '1';
}

async function scanWifiPanel() {
  const sel = byId('wifiScanSel');
  if (!sel) return;
  sel.classList.remove('hidden');
  sel.innerHTML = '<option value="">Buscando...</option>';
  try {
    const r = await fetch('/scanWiFi');
    const j = await r.json();
    sel.innerHTML = '';
    if (!j.networks || !j.networks.length) {
      sel.innerHTML = '<option value="">Sin redes</option>'; return;
    }
    const ph = document.createElement('option');
    ph.value = '';
    ph.textContent = 'Selecciona una red...';
    ph.selected = true;
    ph.disabled = true;
    sel.appendChild(ph);
    j.networks.forEach(n => {
      const o = document.createElement('option');
      o.value = n; o.textContent = n; sel.appendChild(o);
    });
    sel.onchange = selectScannedWifi;
    sel.oninput = selectScannedWifi;
  } catch { sel.innerHTML = '<option value="">Error</option>'; }
}

async function saveWifi() {
  const ssid = (byId('wifiSsid') || {}).value || '';
  const pass = (byId('wifiPass') || {}).value || '';
  const withUser = (byId('wifiWithUser') || {}).checked;
  const wifiUser = withUser ? ((byId('wifiUser') || {}).value || '') : '';
  if (!ssid) { showToast('IngresÃ¡ el nombre de la red', true); return; }
  showToast('Guardando WiFiâ€¦');
  try {
    const body = new URLSearchParams({ ssid, pass, wifiUser });
    const r = await fetch('/saveWiFi', {
      method:'POST',
      headers:{'Content-Type':'application/x-www-form-urlencoded','X-Requested-With':'XMLHttpRequest'},
      body
    });
    if (!r.ok) throw new Error();
    showToast('WiFi guardado â€” reiniciandoâ€¦');
    setText('pWifiSsid', ssid);
    ['wifiSsid', 'wifiPass', 'wifiUser'].forEach(id => { const el = byId(id); if (el) delete el.dataset.userEdited; });
  } catch { showToast('Error al guardar WiFi', true); }
}

async function syncTimePanel() {
  const msg = byId('wifiSyncMsg');
  if (msg) msg.textContent = 'Sincronizandoâ€¦';
  try {
    const d = new Date();
    const body = new URLSearchParams({
      y: d.getFullYear(), mo: d.getMonth()+1, d: d.getDate(),
      h: d.getHours(), mi: d.getMinutes(), s: d.getSeconds(), w: d.getDay()
    });
    const r = await fetch('/syncPhoneTime', {
      method:'POST',
      headers:{'Content-Type':'application/x-www-form-urlencoded','X-Requested-With':'XMLHttpRequest'},
      body
    });
    const j = await r.json().catch(() => null);
    if (r.ok && j && j.ok) {
      if (msg) msg.textContent = 'âœ… Hora sincronizada';
      setText('fechaHora', displayFechaHora(j));
      setText('pRtc', rtcSummary(j));
      setTimeout(loadState, 300);
    } else if (msg) {
      msg.textContent = 'âŒ Error';
    }
  } catch { if (msg) msg.textContent = 'âŒ Error'; }
}

async function connectWifi() {
  const ssid = (byId('wifiSsid') || {}).value || '';
  const pass = (byId('wifiPass') || {}).value || '';
  const withUser = (byId('wifiWithUser') || {}).checked;
  const wifiUser = withUser ? ((byId('wifiUser') || {}).value || '') : '';
  const msgEl = byId('wifiConnectMsg');
  if (!ssid) { showToast('IngresÃ¡ el nombre de la red', true); return; }
  const ok = confirm(
    `Â¿Conectar el Druida a "${ssid}"?\n\n` +
    'Se guardarÃ¡n las credenciales y el dispositivo intentarÃ¡ conectarse.\n' +
    'Si falla, volverÃ¡ al modo AP (192.168.4.1).'
  );
  if (!ok) return;
  if (msgEl) { msgEl.textContent = 'Guardando credencialesâ€¦'; msgEl.classList.remove('hidden'); }
  try {
    const body = new URLSearchParams({ ssid, pass, wifiUser });
    const r1 = await fetch('/saveWiFi', {
      method: 'POST',
      headers: { 'Content-Type': 'application/x-www-form-urlencoded', 'X-Requested-With': 'XMLHttpRequest' },
      body
    });
    if (!r1.ok) throw new Error('saveWiFi');
    if (msgEl) msgEl.textContent = 'Conectandoâ€¦';
    const r2 = await fetch('/connectWiFi', {
      method: 'POST',
      headers: { 'Content-Type': 'application/x-www-form-urlencoded', 'X-Requested-With': 'XMLHttpRequest' },
      body: 'ajax=1'
    });
    const txt = await r2.text().catch(() => '');
    ['wifiSsid', 'wifiPass', 'wifiUser'].forEach(id => { const el = byId(id); if (el) delete el.dataset.userEdited; });
    if (msgEl) msgEl.textContent = r2.ok
      ? 'âœ… ' + (txt || 'Conectando a ' + ssid + 'â€¦')
      : 'âŒ Error al conectar';
    showToast(r2.ok ? 'Conectando a ' + ssid : 'Error al conectar', !r2.ok);
  } catch {
    if (msgEl) msgEl.textContent = 'âŒ Error de conexiÃ³n';
    showToast('Error de conexiÃ³n', true);
  }
}

// =========================
// Alertas
// =========================
const ALERTS_KEY = 'druida_alerts_v1';
function loadAlerts() {
  try {
    const a = JSON.parse(localStorage.getItem(ALERTS_KEY) || '{}');
    const t = id => { const el = byId(id); if (el) el.classList.toggle('on', !!a[id]); };
    t('alertSensor'); t('alertTemp'); t('alertHum');
  } catch {}
}
function saveAlerts() {
  const val = id => !!(byId(id) && byId(id).classList.contains('on'));
  const a = { alertSensor: val('alertSensor'), alertTemp: val('alertTemp'), alertHum: val('alertHum') };
  try { localStorage.setItem(ALERTS_KEY, JSON.stringify(a)); } catch {}
  const body = new URLSearchParams(Object.entries(a).map(([k,v]) => [k, v?'1':'0']));
  fetch('/api/alerts', { method:'POST',
    headers:{'Content-Type':'application/x-www-form-urlencoded','X-Requested-With':'XMLHttpRequest'}, body
  }).catch(() => {});
  showToast('Alertas guardadas');
}


// =========================
// Actualizaciones OTA
// =========================
async function postOTAUpdate(url, label) {
  const msg = byId('updateMsg');

  const ok = confirm(
    label + '\n\n' +
    'El equipo puede reiniciarse durante el proceso.\n' +
    'No cortes la alimentaciÃ³n.\n\n' +
    'Â¿Continuar?'
  );

  if (!ok) return;

  if (msg) msg.textContent = 'Iniciando ' + label + 'â€¦';
  showToast('Iniciando ' + label + 'â€¦');

  try {
    const r = await fetch(url, {
      method: 'POST',
      headers: {
        'X-Requested-With': 'XMLHttpRequest'
      }
    });

    const txt = await r.text();

    if (!r.ok) {
      if (msg) msg.textContent = 'âŒ ' + txt;
      showToast(txt || 'Error en actualizaciÃ³n', true);
      return;
    }

    if (msg) msg.textContent = 'âœ… ' + (txt || label + ' iniciada');
    showToast(txt || label + ' iniciada');

  } catch (e) {
    if (msg) msg.textContent = 'âŒ Error de conexiÃ³n';
    showToast('Error de conexiÃ³n', true);
  }
}

function updateBackend() {
  postOTAUpdate('/updateBackend', 'actualizaciÃ³n Backend');
}

function updateFrontend() {
  postOTAUpdate('/updateFrontend', 'actualizaciÃ³n Frontend');
}

function updateGeneral() {
  postOTAUpdate('/updateGeneral', 'actualizaciÃ³n General');
}

// =========================
// Init
// =========================
document.addEventListener('DOMContentLoaded', () => {
  try { localStorage.removeItem(RELAYCFG_KEY); } catch {}
  ['wifiSsid', 'wifiPass', 'wifiUser'].forEach(id => {
    const el = byId(id);
    if (!el) return;
    el.addEventListener('input', () => { el.dataset.userEdited = '1'; });
  });
  showPreloadedHome();
  loadState();
  setInterval(loadState, 2500);
});
