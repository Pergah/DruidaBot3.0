...
// ── Código único derivado de los primeros 3 bytes de MAC ────
static void computeDeviceCode() {
  if (g_deviceCode.length() > 0) return;
  uint8_t mac[6];
  esp_wifi_get_mac(WIFI_IF_STA, mac);
  char buf[16];
  snprintf(buf, sizeof(buf), "DRUID-%02X%02X-%02X%02X", mac[0], mac[1], mac[2], mac[3]);
...
static String buildStateJson() {
  ...
  j += "\"relays\":{";
  j += relayJson(1, R1estado == LOW, modoR1, horaOnR1, minOnR1, horaOffR1, minOffR1, relayNames[R1name]);
  j += ",";
  j += relayJson(2, R2estado == LOW, modoR2, 0, 0, 0, 0, relayNames[R2name]);
  j += ",";
  j += relayJson(3, R3estado == LOW, modoR3, horaOnR3, minOnR3, horaOffR3, minOffR3, relayNames[R3name]);
  j += ",";
  j += relayJson(4, R4estado == LOW, modoR4, horaOnR4, minOnR4, horaOffR4, minOffR4, relayNames[R4name]);
  j += ",";
  j += relayJson(5, R5estado == LOW, modoR5, horaOnR5, minOnR5, horaOffR5, minOffR5, relayNames[R5name]);
  j += ",";
  j += relayJson(6, R6estado == LOW, modoR6, horaOnR6, minOnR6, horaOffR6, minOffR6, relayNames[R6name]);
  j += ",";
  j += relayJson(7, R7estado == LOW, modoR7, horaOnR7, minOnR7, horaOffR7, minOffR7,
                 (customNameR7[0] != '\0') ? String(customNameR7) : relayNames[R7name]);
  j += ",";
  j += relayJson(8, R8estado == LOW, modoR8, horaOnR8, minOnR8, horaOffR8, minOffR8,
...
static void applyRelayCmd(const String& json) {
  ...
  switch (n) {
    ...
    case 4:
      if (modoNuevo >= 0) modoR4 = modoNuevo;
    ...
    case 7:
      if (modoNuevo >= 0) modoR7 = modoNuevo;
...
// Estado IR A/C (runtime, no persiste)
static uint8_t  g_irAcState     = 0;  // 0=off, 1=cooling
static uint32_t g_irLastTrigger = 0;
#define IR_TRIGGER_MIN_MS 90000UL     // mínimo 90s entre triggers

static void controlarIRac(float temperature, float minVal, float maxVal) {
  if (!g_irConfigured) return;
  if (!temperaturaAmbienteValida(temperature)) return;
  if (!umbralesControlValidos(minVal, maxVal)) return;

  // Relay R2 siempre ON en modo IR (el A/C tiene corriente permanente)
  if (R2estado != LOW) { setRelayCH(RELAY2, true); R2estado = LOW; }

  uint32_t now = millis();
  if ((now - g_irLastTrigger) < IR_TRIGGER_MIN_MS) return;

  bool triggered = false;
  if (temperature >= maxVal && g_irAcState != 1) {
    Serial.printf("[IR] temp %.1f >= %.1f → FRÍO\n", temperature, maxVal);
    if (tuyaTriggerScene(g_irCfg.frio)) { g_irAcState = 1; triggered = true; }
  } else if (temperature <= minVal && g_irAcState != 0) {
    Serial.printf("[IR] temp %.1f <= %.1f → APAGAR\n", temperature, minVal);
    if (tuyaTriggerScene(g_irCfg.off))  { g_irAcState = 0; triggered = true; }
  }
  if (triggered) g_irLastTrigger = now;
}
...
void loop() {
  ...
  // ─── MQTT: mantener conexión y procesar mensajes entrantes ──
...
static void controlarSuperciclo1313(uint8_t relay,
                                    ...
                                    void (*guardar)()) {
  const int32_t tzOffsetSec = -3 * 3600;
...
void handleApiState() {
  ...
  // =========================
  // ESTADO RELÉS — variables software (siempre en sync, evita fallo I2C)
  // =========================
  bool r1On = (R1estado == LOW);
  bool r2On = (R2estado == LOW);
  bool r3On = (R3estado == LOW);
  bool r4On = (R4estado == LOW);
  bool r5On = (R5estado == LOW);
  bool r6On = (R6estado == LOW);
  bool r7On = (R7estado == LOW);
  bool r8On = (R8estado == LOW);
  ...
  json += "\"tempHeaterOn\":"  + String(minR6, 1);
  json += ",\"tempHeaterOff\":" + String(maxR6, 1);
  json += ",\"tempAcOff\":"     + String(minR5, 1);
  json += ",\"tempAcOn\":"      + String(maxR5, 1);
  json += ",\"humHumidOn\":"    + String(minR1, 1);
  json += ",\"humHumidOff\":"   + String(maxR1, 1);
  json += ",\"humDehumOff\":"   + String(minR2, 1);
  json += ",\"humDehumOn\":"    + String(maxR2, 1);
  json += ",\"acRelay\":5";
  json += ",\"heaterRelay\":6";
  json += ",\"humidRelay\":1";
  json += ",\"dehumidRelay\":2";
